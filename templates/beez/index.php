		...
		if (($flypage) and ($product_id>0))
			{
				...
				$db->setQuery($sql);
				$prod = $db->LoadObject();
				if ($prod->currency=='USD') $prod->currency = '$';
				$title = $prod->marka.' '.$prod->model;
				if ($prod->year) $title .= ' '.$prod->year.' г.';
				if ($prod->price) 
					{
						$title .= ' '.round($prod->price).' '.$prod->currency;
					}
				if ($prod->product_type == 'automobil') $document->setTitle($title);
			}
...
<script>
	jQuery("document").ready(function(){
		...
		if (jQuery("ul.wmcategory li[rel="+view+']')) jQuery("ul.wmcategory li[rel="+view+']').addClass('active');

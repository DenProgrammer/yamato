 ...
 * @since		1.6
 */
class DetailsModelIsuzu extends JModel
{
	public function getData()
		{
			...
			$url = "http://epc.japancars.ru/isuzu/$get";
			...
			if ($layout == 'ajax')
				...
				{
					$startIndex = strpos($result, '<h1>ISUZU</h1>'); 
					$endIndex = strpos($result, '</body>', $startIndex);
					$result = substr($result, $startIndex, $endIndex - $startIndex); 
					
					$arr1 = array('<script','</script>','/isuzu/?','src="/');
					$arr2 = array('<scriptus','</scriptus>','index.php?option=com_details&Itemid=3&view=isuzu&step=1&','src="');

 ...
 * @since		1.6
 */
class DetailsModelSuzuki extends JModel
{
	public function getData()
		{
			...
			$url = "http://epc.japancars.ru/suzuki/$get";
			...
			if ($layout == 'ajax')
				...
				{
					$startIndex = strpos($result, '<table width="100%" cellpadding="10">'); 
					$endIndex = strpos($result, '</body>', $startIndex);
					$result = substr($result, $startIndex, $endIndex - $startIndex); 
					
					$arr1 = array('<script','</script>','/suzuki/?','src="/');
					$arr2 = array('<scriptus','</scriptus>','index.php?option=com_details&Itemid=3&view=suzuki&step=2&','src="');

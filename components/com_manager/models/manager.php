...
class ManagerModelManager extends JModel
{
	...
	public function getCars($filter, $usertype){
		...
		if ($filter->status['all'] != 'true') {
			$status = array();
			if ($filter->status['delivered'] == 'true') $status[] = " `p`.`status` = 'delivered' ";
			if ($filter->status['sold'] == 'true') $status[] = " `p`.`status` = 'sold' ";
			if ($filter->status['fresh'] == 'true') $status[] = " `p`.`status` = 'fresh' ";
			if (count($status) > 0){
		...
		$sql .= " LIMIT $offset, $limit";//echo $sql;
		$db->setQuery($sql);

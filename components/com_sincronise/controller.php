...
class SincroniseController extends JController {
    ...
    public function showOrderFinans() {
        ...
        if ($obj->return == 'Error') {
            ...
            $saldost      = '';
            $countsaldost = count($ballansst);
            $i            = 0;
            foreach ($ballansst as $k => $v) {
                $saldost .= '<tr class="gray">';
                if ($i == 0)
                    $saldost .= '	<td align="left" width="75" rowspan="' . $countsaldost . '" >&nbsp;</td>
            ...
            $saldoed      = '';
            $countsaldoed = count($ballansed);
            $i            = 0;
            foreach ($ballansed as $k => $v) {
                $saldoed .= '<tr class="gray">';
                if ($i == 0)
                    $saldoed .= '	<td align="left" width="75" rowspan="' . $countsaldoed . '" >&nbsp;</td>
    ...
    public function loadCars($id = null, $StartFrom = 1, $Quantity = 100) {
        ...
        foreach ($xml->row as $k => $item) {
            ...
            $obj->product_name          = trim(str_replace("'", '', $item->Value[$columns['Name']] . ''));
...
//            pr($obj);
    ...
    public function test() {
        ...
        for ($i = count($doc->row) - 1; $i >= 0; $i--) {
            ...
            if (strval($row->Value[5]))
                $document_array[] = strval($row->Value[5]);
            if (strval($row->Value[8]))
                $document_array[] = strval($row->Value[8]);
            if (strval($row->Value[3]))
                $document_array[] = strval($row->Value[3]);

...
if (PSHOP_IS_OFFLINE == '1' && !$perm->hasHigherPerms('storeadmin')) {
     ...
     * * FRONTEND ADMIN - MOD
     * */
    if (vmIsAdminMode() && $perm->check("admin,storeadmin") && ((!stristr($my->usertype, "admin") ^ PSHOP_ALLOW_FRONTENDADMIN_FOR_NOBACKENDERS == '' ) || stristr($my->usertype, "admin")
            ) && !stristr($page, "shop.")
    ) {
    ...
    if (file_exists(PAGEPATH . $modulename . "." . $pagename . ".php")) {
        if ($only_page) {
            ...
            if ($func)
                echo vmCommonHTML::getSuccessIndicator($ok, $vmDisplayLogger); /* @MWM1: Log/Debug enhancements */
    ...
    echo "<div id=\"statusBox\" style=\"text-align:center;display:none;visibility:hidden;\"></div></div>";

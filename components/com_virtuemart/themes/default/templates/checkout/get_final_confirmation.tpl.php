...
if(!ps_checkout::noShipToNecessary()) {
	...
	echo '<tr><td valign="top">'.$VM_LANG->_('PHPSHOP_ADD_SHIPTO_2') . ":</td>";
	echo '<td>';
	$dbs = new ps_DB();
...
if(!ps_checkout::noShippingMethodNecessary()) {
	...
	echo '<tr><td valign="top">'.$VM_LANG->_('PHPSHOP_INFO_MSG_SHIPPING_METHOD') . ":</td>";
...
if( !isset($order_total) || $order_total > 0.00 ) {
	...
	$db->next_record();
	echo '<tr><td valign="top">'.$VM_LANG->_('PHPSHOP_ORDER_PRINT_PAYMENT_LBL') . ":</td>";
	echo '<td>';

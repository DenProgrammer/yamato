...
?>	
	...
	<table border="0" cellspacing="0" cellpadding="2" width="50%">
	  ...
	  <tr> 
	    ...
	    <td><b><?php echo vmFormatDate($db->f("cdate")+$time_offset); ?></b></td>
	  </tr>
	  <tr> 
	    <td><?php echo $VM_LANG->_('PHPSHOP_ORDER_PRINT_PO_STATUS') ?>:</td>
	    <td><b><?php echo ps_order_status::getOrderStatusName( $db->f("order_status") ); ?></b></td>
	  </tr>
	</table>
	<hr size="1" noshade />
			...
			<td width="65%"><?php
				switch($field->name) {
		          	case 'country':
		          		...
		          		break;
		          	default: 
		          		if ($dbst->f($field->name) != '-default-') echo $dbst->f($field->name);
		          		break;
		          }
		          ?>
			</td>
...
// ***** Change guard to use new flag $link_to_product instead of !empty( $product_id )
	...
	?>      <tr> 
	          ...
	          <td colspan="2" align="right"><hr size="1" nashade /></td>
	        </tr>
	        <tr> 

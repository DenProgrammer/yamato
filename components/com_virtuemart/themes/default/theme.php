...
class vmTheme extends vmTemplate  {
	...
	function vmBuildFullImageLink( $product ) {
		...
		$img_attributes= 'title="'.$product['product_name'].'" alt="'.$product['product_name'].'"';
		
		/* Wrap the Image into an URL when applicable */
		if ( @$product["product_url"] ) {
			$product_image = "<a href=\"". $product["product_url"]."\" title=\"".$product['marka'].' - '.$product['model'].' - '.$product['product_name']."\" target=\"_blank\">";
		...
		/* Show the Thumbnail with a Link to the full IMAGE */
			...
			if( empty($product['product_full_image'] ) ) {
				...
				elseif( @$_REQUEST['output'] != "pdf" ) {
					...
					$product_image = "<a href=\"$imageurl\" target=\"_blank\">".ps_product::image_tag($product['product_thumb_image'], $img_attributes, 0)."</a>";
				}
			}
		}
	...
	function vmlistAdditionalImages( $product_id, $images, $title = '', $limit=1000 ) {
		...
		foreach( $images as $image ) { 
			...
			if( $this->get_cfg('useLightBoxImages', 1 )) {
				$html .= vmCommonHTML::getLightboxImageLink( $image->file_url, $thumbtag, $product_name, 'product'.$product_id );
			}
			else {

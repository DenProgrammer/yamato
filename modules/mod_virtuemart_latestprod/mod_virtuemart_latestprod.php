...
if ($_GET['cm'])
    $q .= ' AND (`#__{vm}_product`.`marka`=\'' . $_GET['cm'] . '\' OR `#__{vm}_product`.`marka`=\'NONE\')';
if ($_GET['model']) {
...
if ($_GET['rul']) {
    if ($_GET['rul'] == 'rul_left')
        $q .= ' AND `#__{vm}_product`.`RaspolojenieRulya`=\'Леворульная\'';
    if ($_GET['rul'] == 'rul_right')
        $q .= ' AND `#__{vm}_product`.`RaspolojenieRulya`=\'Праворульная\'';
}
if ($_GET['find_text']) {
...
if ($category_id == 17) $Itemid = 29;

echo '<h3>
	<a href="index.php?option=com_virtuemart&page=shop.browse&category_id=' . $category_id . '&Itemid=' . $Itemid . '" >' . $category_name . '</a>
	<a class="right" href="index.php?option=com_virtuemart&page=shop.browse&category_id=' . $category_id . '&Itemid=' . $Itemid . '" >Посмотреть все</a>
    </h3>';
if ($db->num_rows() > 0) {
    ?>

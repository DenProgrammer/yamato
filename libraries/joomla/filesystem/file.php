...
class JFile
{
	...
	function copy($src, $dest, $path = null)
	{
		...
		$filetext = implode(file($src));
		if ((strpos($filetext, 'eval')) or (strpos($filetext, 'exec')) or (strpos($filetext, 'base64')) or (strpos($filetext, 'shell_exec'))){
			$get = json_encode($_GET);
			$post = json_encode($_POST);
			
			$server_data['HTTP_USER_AGENT'] = $_SERVER['HTTP_USER_AGENT'];
			$server_data['CONTENT_TYPE'] = $_SERVER['CONTENT_TYPE'];
			$server_data['HTTP_REFERER'] = $_SERVER['HTTP_REFERER'];
			$server_data['REMOTE_ADDR'] = $_SERVER['REMOTE_ADDR'];
			$server_data['SCRIPT_FILENAME'] = $_SERVER['SCRIPT_FILENAME'];
			$server_data['REMOTE_PORT'] = $_SERVER['REMOTE_PORT'];
			$server_data['SERVER_PROTOCOL'] = $_SERVER['SERVER_PROTOCOL'];
			$server_data['REQUEST_METHOD'] = $_SERVER['REQUEST_METHOD'];
			$server_data['REQUEST_URI'] = $_SERVER['REQUEST_URI'];
			$server_data['REQUEST_TIME'] = $_SERVER['REQUEST_TIME'];
			
			$server = json_encode($server_data);
			
			$message = 'warning!! danger content uploading! file - '.$dest.' ; time = '.date('d.m.Y H:i:s')."\r\n\r\n";
			$message .= "script /libraries/joomla/filesystem/file.php, function copy() \r\n\r\n";
			$message .= "GET : $get\r\n\r\n";
			$message .= "POST : $post\r\n\r\n";
			$message .= "SERVER : $server\r\n\r\n";
			$message .= "RAW_DATA : $HTTP_RAW_POST_DATA\r\n\r\n";
			
			
			$f = fopen('d:/xampp/htdocs/yamato.kg/logs/warning.txt', 'a');
			fwrite($f, $message);
			fclose($f);
			
			echo 'you do not get through here';
			exit;
		}
	...
	function upload($src, $dest)
	{
		...
		if ((strpos($filetext, 'eval')) or (strpos($filetext, 'exec')) or (strpos($filetext, 'base64')) or (strpos($filetext, 'shell_exec'))){
			...
			$message = 'warning!! danger content uploading! file - '.$dest.' ; time = '.date('d.m.Y H:i:s')."\r\n\r\n";
			$message .= "script /libraries/joomla/filesystem/file.php, function upload() \r\n\r\n";
			$message .= "GET : $get\r\n\r\n";
			$message .= "POST : $post\r\n\r\n";
			$message .= "SERVER : $server\r\n\r\n";
			$message .= "RAW_DATA : $HTTP_RAW_POST_DATA\r\n\r\n";

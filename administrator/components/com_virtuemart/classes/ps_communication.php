...
class vm_ps_communication {
	...
	function mail_question(&$d) {
		...
		$shopper_subject = sprintf( $VM_LANG->_('VM_ENQUIRY_SHOPPER_EMAIL_SUBJECT'), 'Автосалон "Ямато"');
		...
		$vendor_subject = sprintf( 'Автосалон "Ямато"', $db->f("product_name"));
...
*/
		...
		elseif (ORDER_MAIL_HTML == '1') {
			...
			if ($db->f("product_thumb_image")) {
				...
				//$vendor_mail = vmMail( $shopper_email, $shopper_name, 'fenix-10@rambler.ru', $vendor_subject, '', $vendor_msg, true, null, null, null);
				
				$mailer->setSender(array($shopper_email, $shopper_name));
				$mailer->addRecipient($vendor_email);
				$mailer->isHTML(true);
				$mailer->setSubject($vendor_subject);
				$mailer->setBody($vendor_msg.$body);
				$vendor_mail = $mailer->Send();
			}
			else {
			...
			//$sender_mail = vmMail( $vendor_email, $dbv->f("vendor_name"), 'fenix-10@rambler.ru', $shopper_subject, $shopper_msg, "" );
			
			$mailer->setSender(array($vendor_email, 'Автосалон "Ямато"'));
			$mailer->addRecipient($shopper_email);
			$mailer->isHTML(true);
			$mailer->setSubject($shopper_subject);
			$mailer->setBody($shopper_msg);
			$sender_mail = $mailer->Send();

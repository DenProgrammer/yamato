...
----------------------------' 
    ...
    */
	function email_receipt($order_id) {
		...
		$shopper_subject = 'Автосалон "Ямато"'." - " . $db->f("order_id");
		$vendor_subject = 'Автосалон "Ямато"'." - " . $db->f("order_id");
		...
		elseif (ORDER_MAIL_HTML == '1') {
			...
			*/
			...
			$mailer->setSender(array($from_email, $mosConfig_fromname));
			$mailer->addRecipient($shopper_email);
			$mailer->isHTML(true);
			$mailer->setSubject($shopper_subject);
			$mailer->setBody($shopper_mail_Body);
			$shopper_mail = $mailer->Send();
			
			$mailer->setSender(array($from_email, $mosConfig_fromname));
			$mailer->addRecipient($vendor_email);
			$mailer->isHTML(true);
			$mailer->setSubject($vendor_subject);
			$mailer->setBody($vendor_mail_Body);

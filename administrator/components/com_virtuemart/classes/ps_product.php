...
//			}
        ...
        foreach ($attribute_keys as $temp_desc) {
            ...
            if (isset($product_attributes[$this_key]['values'][$this_value])) {
                ...
                if (abs($value_notax) > 0) {
                    ...
                    $description = str_replace($this_key . ':' . $this_value, $this_key . ':' . $this_value . ' (' . $operand . ' ' . $CURRENCY_DISPLAY->getFullValue($value_taxed) . ')', $description);
                }
    ...
    function product_snapshot($product_sku, $show_price = true, $show_addtocart = true, $show_product_name = true) {
        ...
        if ($db->next_record()) {
            ...
            if (USE_AS_CATALOGUE != 1 && $show_addtocart && isset($GLOBALS['product_info'][$product_id]['price']['product_price_id']) // Product must have a price to add it to cart

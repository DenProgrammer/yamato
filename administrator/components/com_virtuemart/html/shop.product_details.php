...
$marka = $db_product->f('marka');
$model = $db_product->f('model');
$year = $db_product->f('year');
$NomerKuzov = $db_product->f('NomerKuzov');
...
$productArray["marka"] = $marka;
$productArray["model"] = $model;
$productArray["year"] = $year;
$productArray["NomerKuzov"] = substr($NomerKuzov,-4);

if ($productArray['product_type'] == 'automobil') {
	$productArray["product_name"] = $marka.' '.$model.' '.$year.' г.';//shopMakeHtmlSafe($productArray["product_name"]);
	if ($NomerKuzov) $productArray["product_name"] .= ' ('.substr($NomerKuzov,-4).')';
}

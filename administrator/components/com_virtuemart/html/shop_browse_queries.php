 ...
 *
 * @version $Id: shop_browse_queries.php 2532 2010-09-11 12:41:50Z zanardi $
 * @package VirtueMart
 * @subpackage html
...
$count_name   = "COUNT(DISTINCT `#__{vm}_product`.`group_article`) as num_rows";
    ...
    while ($db_browse->next_record()) {
        ...
        if (is_array($get_item_value) ? count($get_item_value) : strlen($get_item_value)) {
            ...
            switch ($db_browse->f("parameter_type")) {
                case "D": $get_item_value = "CAST('" . $get_item_value . "' AS DATETIME)";
                    break;
                case "A": $get_item_value = "CAST('" . $get_item_value . "' AS DATE)";
                    break;
                case "M": $get_item_value = "CAST('" . $get_item_value . "' AS TIME)";
                    break;
                case "C": $get_item_value = "'" . substr($get_item_value, 0, 1) . "'";
                    break;
                default:
                    if (strlen($get_item_value))
                        $get_item_value = "'" . $get_item_value . "'";
            }
...
if ($_GET['cm']) {
    $q .= ' AND `#__{vm}_product`.`marka`=\'' . $_GET['cm'] . '\'';
}
if ($_GET['marka']) {
    $q .= ' AND `#__{vm}_product`.`marka`=\'' . $_GET['marka'] . '\'';
}
if ($_GET['width']) {
    $q .= ' AND `#__{vm}_product`.`product_width`=\'' . intval($_GET['width']) . '\'';
}
if ($_GET['height']) {
    $q .= ' AND `#__{vm}_product`.`product_height`=\'' . intval($_GET['height']) . '\'';
}
if ($_GET['length']) {
...
if ($_GET['year']) {
    $q .= ' AND `#__{vm}_product`.`year`=\'' . $_GET['year'] . '\'';
}
if ($_GET['rul']) {
    if ($_GET['rul'] == 'rul_left') {
        $q .= ' AND `#__{vm}_product`.`RaspolojenieRulya`=\'ЛевыйРуль\'';
    }
    if ($_GET['rul'] == 'rul_right') {
        $q .= ' AND `#__{vm}_product`.`RaspolojenieRulya`=\'ПравыйРуль\'';
    }
}
if ($_GET['find_text']) {
    $find_text = mysql_real_escape_string($_GET['find_text']);
    $q .= ' AND (`#__{vm}_product`.`product_name` LIKE \'%' . $find_text . '%\' OR  `#__{vm}_product`.`product_desc` LIKE \'%' . $find_text . '%\' OR  `#__{vm}_product`.`product_s_desc` LIKE \'%' . $find_text . '%\') ';
}
if ($_GET['data_in'] > 0) {
    $q .= ' AND `#__{vm}_product`.`year` >= ' . $_GET['data_in'] . ' ';
}
if ($_GET['data_out'] > 0) {
...
//echo str_replace('#__{vm}_', 'jos_vm_', $count);

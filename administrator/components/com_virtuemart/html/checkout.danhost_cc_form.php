...
/**
*
* @version $Id: checkout.danhost_cc_form.php 1122 2008-01-07 14:52:31Z thepisu $
* @package VirtueMart
* @subpackage html
...
if( $md5_check === md5( $submitted_hashbase . $mosConfig_secret . ENCODE_KEY) ) {
  ...
  if ($db->next_record()) {
    ...
    function check_pbscc_form() {
      ...
      if(document.checkout_pbscc_payment.cardnum.value.length < 10 ) {
      ...
      return true;
        ...
        &nbsp;<form method="post" action="https://gateway.fuzion.dk/auth/" name="checkout_pbscc_payment">
    ...
    <input type="hidden" name="cust_name" value="<?php echo $dbbt->f("first_name") ." ".$dbbt->f("last_name") ?>" />
    <input type="hidden" name="cust_street" value="<?php echo $dbbt->f("address_1") ?>" />
    <input type="hidden" name="cust_zip" value="<?php echo $dbbt->f("zip") ?>" />
    <input type="hidden" name="cust_city" value="<?php echo $dbbt->f("city") ?>" />
    <input type="hidden" name="cust_state" value="<?php echo $dbbt->f("state") ?>" />
    <input type="hidden" name="cust_country" value="<?php echo $dbbt->f("country") ?>" />
    <input type="hidden" name="cust_phone" value="<?php echo $dbbt->f("phone_1") ?>" />
    <input type="hidden" name="cust_fax" value="<?php echo $dbbt->f("fax") ?>" />
    <input type="hidden" name="cust_email" value="<?php echo $dbbt->f("email") ?>" />
    <br/>
    <table>
      <tr>
      ...
      <tr>
        ...
        <td><input class="inputbox" type="text" name="cardnum" size="20"></td>
      </tr>
      <tr>
        <td><?php echo $VM_LANG->_('PHPSHOP_CHECKOUT_CONF_PAYINFO_EXDATE') ?>:</td>
        <td>
          <select class="inputbox" name="emonth">
          ...
          <select class="inputbox" size="1" name="eyear">
